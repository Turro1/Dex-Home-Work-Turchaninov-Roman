    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (Client client in clients)
            {
                ...
                int sum = clients.Sum(client.check);
                int min = clients.Min(client.check);
                Console.WriteLine($"Сумма всех денег в банке{sum}");
                Console.WriteLine($"Минимальный счёт в банке{sum}");
                client.Display();
                Console.ReadKey();
            }
